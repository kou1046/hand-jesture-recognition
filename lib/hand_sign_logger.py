...
class HandSignDataLoggerGUI:
    def __init__(self, store: LabeledHandPointsStore = NdJsonLabeledHandPointsStore(os.path.join("models", "labeled_handpoints.ndjson"))):
        self.store = store
    ...
    def _display(self, img: np.ndarray):
        ...
        cv2.putText(img, display_text, (0, 50),
                   cv2.FONT_HERSHEY_SIMPLEX, 1, text_color, 1, cv2.LINE_AA)
        cv2.imshow("img", img)
        key = cv2.waitKey(1)
        if key == 27: #Esc
